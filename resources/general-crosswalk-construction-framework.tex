...
\item Compute all weights for all atoms of interest: $\bm{w_{\mathbf{CST}}} = \displaystyle \frac{\bm{c_{st}}}{\bm{c_{s}}}$.
  ...
  \end{enumerate}
\item If $\bm{w_{\mathbf{cst}}}$ is missing data for any $\bm{s}$ in $\mathbf{S}$ or $\bm{t}$ in $\mathbf{T}$, add dummy atoms with \textit{null} $\bm{t}$ for non-\textit{null} $\bm{s}$ or \textit{null} $\bm{s}$ for non-\textit{null} $\bm{t}$, and set wCst = 0.
